...
def test_pre_commit_hooks(install_package):
    ...
    assert result.returncode == 0, f"Pre-commit failed with output: {result.stdout}"


def test_bump_version_major(install_package):
    # Bump version major
    result = subprocess.run(["bump2version", "major", "--dry-run"], capture_output=True, text=True)
    ...
    # Check that the version file would be bumped (check for expected version change in the output)
    assert "bumped version" in result.stdout, "Version bump did not happen"

    # Optionally, check the version string in `__init__.py` or version file to see if it's changed
    with open("templatepy/__init__.py", "r") as f:
        version = f.read()
        assert "0.0.4" in version  # Example, this should match the bumped version.


def test_bump_version_minor(install_package):
    # Bump version minor
    result = subprocess.run(["bump2version", "minor", "--dry-run"], capture_output=True, text=True)
...
def test_bump_version_patch(install_package):
    # Bump version patch
    result = subprocess.run(["bump2version", "patch", "--dry-run"], capture_output=True, text=True)
